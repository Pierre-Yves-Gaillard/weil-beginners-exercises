# https://github.com/Pierre-Yves-Gaillard/weil-beginners-exercises
# https://shorturl.at/2PrmJ
# https://claude.ai/chat/7c1796ac-d088-493e-b106-f1df0431e5d9
#!/usr/bin/env python3
"""
...
extended Euclidean algorithm applied to two integers a0 > a1 > 0.
  ...
  Here "round" means round to the nearest integer, with ties (i.e.
  a value of the form k + 1/2 for an integer k) broken by rounding
  to the nearest EVEN integer -- this is "round-half-to-even", a.k.a.
  banker's rounding (so e.g. round(5/2) = 2, round(7/2) = 4,
  round(-5/2) = -2). This is implemented EXACTLY, using Python's
  fractions.Fraction together with the built-in round() function --
  no floating-point arithmetic is used anywhere in this computation,
  so there is no risk of floating-point rounding error changing the
  result. Because q_i is now defined by rounding rather than by
  floor(), the a_i, x_i, y_i, q_i can be negative from i = 2 onward
  (a0, a1 are still assumed positive).
...
def round_half_to_even(numerator: int, denominator: int) -> int:
    """
    Return the integer nearest to numerator/denominator, rounding a
    value of the form k + 1/2 (k an integer) to the nearest EVEN
    integer (banker's rounding / round-half-to-even). Exact: uses
    Python's Fraction, so there is no floating-point error, and it
    works correctly regardless of the signs of numerator/denominator.
    """
...
def extended_euclid_table(a0: int, a1: int):
    ...
    q_i is round(a_{i-1}/a_i) (round-half-to-even for ties -- see
    round_half_to_even above).
    ...
    # i = 1
    q_prev = round_half_to_even(a_prev2, a1)   # q1 = round(a0/a1)
    ...
    while True:
        ...
        q_i = round_half_to_even(a_prev1, a_i)   # round(a_{i-1}/a_i)
